<template>
  ...
  import ComplaintTypeCreateModal from "@/components/complainantType/ComplaintTypeCreateModal.vue";
  import ComplaintTypeEditModal from "@/components/complainantType/ComplaintTypeEditModal.vue";
  ...
  export default {
    ...
    data() {
      return {
        fields: [
          { key: "id", label: "Id" },
          { key: "name", label: "Complainant Type" },
        ...
        ComplainantCreateModalCount: 0,
        ComplainantTypeEditModalCount: 0,
      };
    },
    mixins: [util],
    async mounted() {
    ...
    methods: {
      ...mapActions({
        ...
        deleteComplaintType: "appData/deleteComplaintType",
      }),
      async fetchPaginatedData() {
        try {
          ...
          this.ComplainantTypes = res.data.results;
      ...
      editPetitionType(ComplainantType) {
        this.ComplainantType = ComplainantType;
        this.ComplainantTypeEditModalCount += 1;
        this.$nextTick(() => {
          this.$bvModal.show("complaint-type-edit-modal");
        });
      },
      async removeComplaintType(ComplainantType) {
        this.ComplainantType = ComplainantType;
        try {
          const res = await this.deleteComplaintType({
            pk: this.ComplainantType.id,
            
          });
          
          if (res.status === 204) {
            this.$swal({
              title: "Complainant Type deleted successfully",
              icon: "success",
            });
            if (
