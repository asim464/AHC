<template>
    <b-modal
      id="Corps-create-modal"
      title="Corps Create Modal"
      centered
      hide-footer
      size="lg"
      @hidden="reset"
  ...
  export default {
    ...
    methods: {
      ...mapActions({
        createCorps: "appData/createCorps",
      }),
      ...
      async submit() {
        try {
          const res = await this.createCorps({
            name: this.type,
          ...
          if (res.status === 201) {
            this.$swal({
              title: "Corps created successfully",
              icon: "success",
            });
            this.reset();
            this.$nextTick(() => {
              this.$bvModal.hide("Corps-create-modal");
            });
