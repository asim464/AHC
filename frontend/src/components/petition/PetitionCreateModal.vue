...
<template>
  <b-modal
    id="petition-create-modal"
    title="RFA Create Modal"
    centered
    hide-footer
    size="xl"
  ...
  >
    <template #modal-title> <h2 class="m-0">Create Complaint</h2> </template>
      ...
      >
        <b-tabs content-class="py-1" fill>
          <!-- <b-tab no-body>
            ...
            <validation-observer ref="petitionCreateHomeFormValidation">
              <PetitionHomeForm ref="homeForm" :mode="modes.CREATE" />
            </validation-observer>
          </b-tab> -->
          <!-- <b-tab no-body> -->
            <template #title>
            ...
            <validation-observer ref="petitionCreateDetailFormValidation">
              <PetitionDetailForm ref="detailForm" :mode="modes.CREATE" />
            </validation-observer>
          <!-- </b-tab> -->
        </b-tabs>
      </vue-perfect-scrollbar>
  ...
  methods: {
    ...
    async saveDraft() {
      ...
      try {
        ...
        if (res.status === 201) {
          this.$swal({
            title: "RFA created successfully",
            icon: "success",
          });
          this.$nextTick(() => {
    ...
    async validationForm() {
      let success =
        await this.$refs.petitionCreateHomeFormValidation.validate();
      if (success) {
        this.homeFormState = false;
        success =
          await this.$refs.petitionCreateDetailFormValidation.validate();
        if (success) {
    ...
    async submit() {
      ...
      try {
        ...
        for (var i = 0; i < homeForm.markFiles.length; i++) {
          let file = homeForm.markFiles[i];

          formData.append("markFiles[" + i + "]", file);
          fileData["markFiles[" + i + "]"] = {
            is_marked: true,
          };
        }
